...
function InstallRequiredModules {
    if(-not (Get-Module -Name "AzureRM")) {
        Install-Module -Name AzureRM -RequiredVersion 6.13.1 -Force
    }
}
...
function ProvisionLabVirtualMachines {
    ...
    New-AzureRmResourceGroupDeployment -Name $vmDeploymentName `
        -ResourceGroupName $ResourceGroupName `
        -Force `
...
# This runs each deployment of a vm as a job and doesn't wait for it to finish provisioning
